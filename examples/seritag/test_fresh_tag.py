    ...
    try:
        with CardManager(0) as card:
            ...
            try:
                ...
                if e.sw2 == 0x85:  # Conditions not satisfied
                    print("[INFO] SelectPICC rejected (SW=6985) - tag may be in different state")
            ...
            print(f"     Hex: {data_bytes.hex().upper()}")
            
            # Collect all frames
            data_bytes = bytes(data)
            full_response = bytearray(data_bytes)
            frame_count = 1
            ...
            while (sw1, sw2) == SW_ADDITIONAL_FRAME:
                ...
                if (sw1, sw2) == SW_ADDITIONAL_FRAME or (sw1, sw2) == SW_OK:
                    full_response.extend(bytes(data))
