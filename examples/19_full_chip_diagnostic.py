...
def read_file_list(card):
    ...
    print("[INFO] Note: Fresh tags typically have no files created yet")
    ...
    for file_id in known_files:
        try:
            ...
            if e.sw2 == 0xF0:  # File not found
                print(f"  [OK] File 0x{file_id:02X}: does not exist (expected for fresh tag)")
                ...
                # Unknown error - likely means file doesn't exist
                pass
    
    if not existing_files:
        print("\n[INFO] No files found - this appears to be a fresh/uninitialized tag")
        print("       Files need to be created before they can be read")
...
def read_key_versions(card, session: Optional[Ntag424AuthSession] = None):
    ...
    print("[INFO] Note: GetKeyVersion requires CommMode.MAC authentication")
    print("[INFO] Fresh tags may have keys but GetKeyVersion may still fail")
    print("[INFO] Errors may indicate command counter or CMAC calculation issues\n")
    
    for key_no in range(5):  # Keys 0-4
        print(f"Key 0x{key_no:02X}: ", end="")
        ...
        try:
            ...
            elif e.sw2 == 0x1E:  # Integrity Error - CMAC validation failed
                print(f"[WARN] CMAC validation failed (SW={e.sw1:02X}{e.sw2:02X})")
                print(f"       This may indicate a command counter issue")
            elif e.sw2 == 0x7E:  # Length Error
                print(f"[WARN] Length error (SW={e.sw1:02X}{e.sw2:02X})")
                print(f"       Command format may be incorrect")
            else:
                print(f"[ERROR] {e} (SW={e.sw1:02X}{e.sw2:02X})")
        except Exception as e:
            print(f"[ERROR] {e}")
...
def main():
    ...
    print("\nNote: For fresh/uninitialized tags, most files won't exist yet.")
    print("      Some commands may fail, which is expected behavior.\n")
    ...
    try:
        with CardManager() as card:
            ...
            try:
                ...
                if len(file_ids) == 0:
                    print("    (Tag appears to be fresh/uninitialized - no files created yet)")
                print(f"  - Authentication: SUCCESS")
                print(f"  - All readable data retrieved")
                if len(file_ids) == 0:
                    print("\n[INFO] Next steps for a fresh tag:")
                    print("       1. Create NDEF file (CreateStdDataFile)")
                    print("       2. Write NDEF message (WriteData)")
                    print("       3. Configure SDM (ChangeFileSettings) if needed")
