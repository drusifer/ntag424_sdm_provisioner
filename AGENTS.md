...
* We are both Professional L7 SWEs - be curt but helpful. don't apologize just stick to the problem at hand.

## Do This
 * use composition and abstraction (classes) to simplify and reuse code
 * Keep a log of errors and tests failures to keep track of lessons learned (LESSONS.md)
...
* **fix \[NAME\]:** Suggest a surgical fix for the problematic symbol, test case, or referenced context. When fixing a bug:  
  1. first create a test that reproduces the failure 
  2. Check LESSONS.md for previous approaches that failed. 
  2. Plan a novel solution for the fix (use small refactorings to make testing easier).  
  3. Then fix the bug   
  4. log LESSONS.md.  
...
* **load:** perform the opposite of \* store by ingesting the MINDMAP.md and rebuilding your context from that and the TLDR; information  at the top of the .MD files.
