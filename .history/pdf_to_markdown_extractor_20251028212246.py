...
class EnhancedPDFExtractor:
    ...
    def analyze_page_content(self, page: fitz.Page, page_num: int) -> ExtractedContent:
        ...
        # Extract text
        text = page.get_text()
        has_meaningful_text = len(text.strip()) > 50 and not self._is_mostly_gibberish(text)
    ...
    def _is_mostly_gibberish(self, text: str) -> bool:
        ...
        return (readable_chars / total_chars) < 0.7 if total_chars > 0 else True
...
```"""
    ...
    def process_page(self, page: fitz.Page, page_num: int) -> str:
        ...
        # Add text content (cleaned up)
        if text_content:
            # Clean up text
            cleaned_text = re.sub(r'\n\s*\n\s*\n', '\n\n', text_content)  # Remove excessive newlines
            cleaned_text = re.sub(r'[ \t]+', ' ', cleaned_text)  # Normalize whitespace
        ...
        if content.diagrams:
            ...
            for i, diagram in enumerate(content.diagrams):
                md_lines.append(f"#### {diagram['type'].title()} Diagram {i+1}\n")
                mermaid_diagram = self.generate_mermaid_diagram(diagram, text_content)
                md_lines.append(mermaid_diagram)
                md_lines.append("")
        
        # Handle page visualization - either inline SVG or extracted images
        if self.use_inline_svg:
            # For pages with minimal text or many graphics, show full page as inline SVG
            if not content.has_text or len(content.images) > 2:
    ...
    def extract(self):
        ...
        md_lines = [
            ...
            f"- **Extraction Method:** Text + OCR + Image extraction",
            f"- **Enhanced Features:** Mermaid diagrams, Markdown tables, PNG images",
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python pdf_to_markdown_extractor.py <pdf_file> [output_name]")
        print("\nFeatures:")
        ...
        print("  • PNG image extraction")
        print("\nExample: python pdf_to_markdown_extractor.py API-ACR122U-2.04.pdf acr122u-spec")
        return
    ...
    try:
        extractor = EnhancedPDFExtractor(pdf_file, output_name)
        success = extractor.extract()
        
        if success:
            log.info("✅ Enhanced extraction successful!")
            log.info("🔍 Check the markdown file for structured content with mermaid diagrams")
