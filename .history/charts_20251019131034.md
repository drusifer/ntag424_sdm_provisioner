...
```mermaid
flowchart TD
    A["ATR for ISO 14443 Part 3 PICCs"] --> B["Initial Header (3Bh): 8 bits"]
    A --> C["T0: 8 bits"]
    A --> D["TD1: 8 bits"]
    A --> E["TD2: 8 bits"]
    A --> F["T1 (Category Indicator): 8 bits"]
    A --> G["Historical Bytes (N bytes)"]
    G --> H["Tk (Application Identifier): 16 bits"]
    G --> I["RID (A000000306h): 40 bits"]
    G --> J["Standard (S): 8 bits"]
    G --> K["Card Name (C0, C1): 16 bits"]
    G --> L["RFU: 32 bits"]
    A --> M["TCK (Checksum): 8 bits"]
```
...
```mermaid
flowchart TD
    A["ATR for ISO 14443 Part 4 PICCs"] --> B["Initial Header (3Bh): 8 bits"]
    A --> C["T0: 8 bits"]
    A --> D["TD1: 8 bits"]
    A --> E["TD2: 8 bits"]
    A --> F["Historical Bytes (from ATS/ATQB response)"]
    F --> G["T1 .. Tk: ..."]
    A --> H["TCK (Checksum): 8 bits"]
```
...
Class (FFh):     8 bits
INS (CAh):       8 bits  
P1 (00h=UID, 01h=ATS): 8 bits
P2 (00h):        8 bits
Le (Expected Length): 8 bits
```
...
Class (FFh):     8 bits
INS (82h):       8 bits
P1 (Key Structure): 8 bits
P2 (Key Number): 8 bits
Lc (06h):        8 bits
Data In (Key):   48 bits
```
...
Data In:
  ...
  00h:           8 bits
  Block Number:  8 bits
  Key Type (60h=A, 61h=B): 8 bits
  Key Number (00h-01h): 8 bits
```
...
INS (B0h):       8 bits
P1 (00h):        8 bits
P2 (Block Number): 8 bits
Le (Bytes to Read): 8 bits
```
...
**Response Structure:**
```
Data Out (N bytes): Variable length
SW1:               8 bits
SW2:               8 bits
```
