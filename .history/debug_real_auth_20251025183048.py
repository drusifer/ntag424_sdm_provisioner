...
def debug_real_auth():
    ...
    try:
        with CardManager(0) as card:
            ...
            # Step 1: Try SelectPiccApplication (optional)
            print("\nStep 1: SelectPICCApplication (optional)")
            try:
                result = SelectPiccApplication().execute(card)
                print(f"SelectPICCApplication: {result}")
            except Exception as se:
                print(f"SelectPICCApplication failed (this might be OK): {se}")
                print("Continuing without application selection...")
