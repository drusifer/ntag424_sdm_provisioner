...
def format_file_settings(data: bytes) -> str:
    """Format file settings data into readable text."""
    if len(data) < 7:
        return f"<incomplete: {len(data)} bytes>"
    ...
    lines = [
        f"  File Type: 0x{file_type:02X}",
        f"  File Option: 0x{file_option:02X}",
        f"  Access Rights: {format_bytes(access_rights)}",
        f"  File Size: {file_size} bytes",
    ]
    
    # Parse SDM options if present
    if len(data) > 7:
        offset = 7
        if file_option & 0x40:  # SDM enabled (bit 6)
            if offset < len(data):
                sdm_options = data[offset]
                offset += 1
                lines.append(f"  SDM Options: 0x{sdm_options:02X}")
            
            if offset + 2 <= len(data):
                sdm_access_rights = data[offset:offset+2]
                offset += 2
                lines.append(f"  SDM Access Rights: {format_bytes(sdm_access_rights)}")
            
            # UID offset
            if offset + 3 <= len(data):
                uid_offset = int.from_bytes(data[offset:offset+3], 'little')
                offset += 3
                lines.append(f"  UID Offset: {uid_offset}")
            
            # SDM Read Counter offset
            if offset + 3 <= len(data):
                ctr_offset = int.from_bytes(data[offset:offset+3], 'little')
                offset += 3
                lines.append(f"  Read Counter Offset: {ctr_offset}")
            
            # PICC Data offset
            if offset + 3 <= len(data):
                picc_offset = int.from_bytes(data[offset:offset+3], 'little')
                offset += 3
                lines.append(f"  PICC Data Offset: {picc_offset}")
            
            # MAC Input offset
            if offset + 3 <= len(data):
                mac_input_offset = int.from_bytes(data[offset:offset+3], 'little')
                offset += 3
                lines.append(f"  MAC Input Offset: {mac_input_offset}")
...
def read_file_list(card):
    """Read list of files (no authentication required)."""
    ...
    try:
        ...
        if not file_ids:
            print("[INFO] No files found in application")
            return []
...
def read_file_settings(card, file_ids: List[int], session: Optional[Ntag424AuthSession] = None):
    ...
    for file_id in file_ids:
        ...
        try:
            ...
            settings_data = cmd.execute(card)
            
            print("[OK] File settings retrieved")
            print(format_file_settings(settings_data))
            print(f"  Raw Data ({len(settings_data)} bytes): {format_bytes(settings_data, max_len=64)}")
...
def read_key_versions(card, session: Optional[Ntag424AuthSession] = None):
    ...
    for key_no in range(5):  # Keys 0-4
        ...
        try:
            ...
            version = cmd.execute(card)
            print(f"[OK] Version: 0x{version:02X}")
        except ApduError as e:
