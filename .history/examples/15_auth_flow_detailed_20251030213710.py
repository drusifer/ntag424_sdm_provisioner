...
def main():
    ...
    try:
        with CardManager(0, timeout_seconds=15) as card:
            ...
            if is_seritag and success_count == 0:
                ...
                try:
                    ...
                    print(f"\n[SUCCESS] HIGH-LEVEL AUTH SUCCESSFUL!")
                except ApduError as e:
                    print(f"\n[ERROR] High-level auth failed (expected): {e}")
                    print(f"   SW: {e.sw1:02X}{e.sw2:02X}")
            ...
            print("  1. [OK] Phase 1 using AuthenticateEV2First command class")
            print("  2. [OK] Phase 2 using AuthenticateEV2Second command class")
            print("  3. [OK] Multiple key derivation methods tested")
            print("  4. [OK] High-level API using Ntag424AuthSession")
        ...
        print(f"\n[ERROR] APDU ERROR: {e}", file=sys.stderr)
        print(f"   SW: {e.sw1:02X}{e.sw2:02X}")
        return 1
    except Exception as e:
