    ...
    def run_all_tests(self):
        ...
        try:
            ...
            for p1_mode in [0x02, 0x00]:  # Try P1=0x02 first (select EF under current DF), then 0x00
                ...
                if select_success:
                    print(f"[OK] File selection works with {p1_name}: SW={sw1:02X}{sw2:02X}")
                    break
                else:
                    print(f"[FAIL] File selection failed with {p1_name}: SW={sw1:02X}{sw2:02X}")
            print()
