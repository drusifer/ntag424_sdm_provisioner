...
class ComprehensiveNdefTest:
    ...
    def setup(self):
        ...
        self.card = CardManager(0).__enter__()
    ...
    def teardown(self):
        """Close connection."""
        if self.card and hasattr(self.card, '__exit__'):
    ...
    def run_all_tests(self):
        ...
        try:
            ...
            # Test file selection first
            print("Testing NDEF file selection...")
            select_success, sw1, sw2 = self.select_ndef_file(use_escape=True)
            if select_success:
                print(f"✅ File selection works: SW={sw1:02X}{sw2:02X}")
            else:
                print(f"❌ File selection failed: SW={sw1:02X}{sw2:02X}")
            print()
            ...
            # ReadData: 90 AD 00 00 [Lc] [FileNo OffsetHigh OffsetLow 00 LenHigh LenLow 00] [Data] 00
            try:
                read_data_apdu = [
                    0x90, 0xAD,  # ReadData
                    0x00, 0x00,  # P1, P2
                ...
                data, sw1, sw2 = self.card.connection.send_apdu(read_data_apdu, use_escape=True)
                if (sw1, sw2) == SW_OK:
                    result = TestResult(
