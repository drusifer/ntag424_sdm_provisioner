...
def test_phase2_with_logging(card):
    ...
    if (sw1_2, sw2_2) == SW_OK:
        ...
        # Additional analysis: Try to understand what went wrong
        print("\n" + "-" * 80)
        print("STEP 10: Detailed Analysis")
        print("-" * 80)
        
        print("\nPossible reasons for SW=91AE (Wrong RndB'):")
        print("1. Wrong key - but we're using factory key (all zeros)")
        print("2. Wrong RndB extraction - but we got 16 bytes from Phase 1")
        print("3. Wrong RndB rotation - but we're rotating left by 1 byte (per spec)")
        print("4. Wrong encryption format - but we're using AES-ECB (per spec)")
        print("5. Seritag protocol difference - most likely")
        
        print("\nNext steps to try:")
        print("- Test different keys (UID-based, all-ones, etc.)")
        print("- Test different RndB rotation (maybe right rotate or no rotate?)")
        print("- Test different encryption modes")
        print("- Review response timing (maybe need delay?)")
    ...
    try:
        with CardManager(0, timeout_seconds=15) as card:
            ...
            print("\n" + "=" * 80)
                    ...
                    # Fresh Phase 1 for Phase 2 test (need new transaction)
            print("\nGetting fresh Phase 1 challenge for Phase 2 test...")
            apdu_fresh = [0x90, 0x71, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00]
            data_fresh, sw1_fresh, sw2_fresh = card.send_apdu(apdu_fresh, use_escape=True)
            if (sw1_fresh, sw2_fresh) != SW_ADDITIONAL_FRAME:
                print(f"[WARN] Fresh Phase 1 failed, but continuing...")
