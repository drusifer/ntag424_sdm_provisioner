...
class ComprehensiveNdefTest:
    ...
    def setup(self):
        ...
        if USE_MOCK_HAL:
            ...
            print("🔧 Using MOCK HAL for testing")
        else:
            self.card_manager = CardManager(0)
            print("🔌 Using REAL HAL (hardware required)")
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    if USE_MOCK_HAL:
        print("🔧 Running with MOCK HAL (no hardware required)")
        print("   Set USE_MOCK_HAL=0 to use real hardware")
    else:
        print("🔌 Running with REAL HAL (hardware required)")
