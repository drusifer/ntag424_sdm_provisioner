...
def try_command(card, name, apdu, description=""):
    ...
    try:
        ...
        if (sw1, sw2) in [SW_OK, SW_ADDITIONAL_FRAME]:
            status = "[OK] " + status
...
def diagnostic_scan(card):
    ...
    # Note: This requires authentication with MAC mode
    ...
    for file_no in files_to_check:
        ...
        if success and data:
            ...
            if not data or len(data) == 0:
                print(f"  [NOTE] SW=9000 but no data returned - may need authentication")
            else:
                print(f"  [NOTE] Unexpected: SW=9000 with data")
    ...
    proprietary_commands = [
        ...
        (0x51, "GetCardUID (0x51 - requires auth)"),
        (0x52, "Command 0x52"),
        (0x53, "Command 0x53"),
    ]
    ...
    # 7. GetCardUID (0x51) - Requires authentication
    print_section("7. GET CARD UID (Command 0x51)")
    print("Note: This command requires authentication (CommMode.Full)")
    # GetCardUID: 90 51 00 00 00
    apdu = [0x90, 0x51, 0x00, 0x00, 0x00]
    success, data, sw1, sw2, status = try_command(card, "GetCardUID", apdu)
    print(f"{status}")
    if success and data:
        print(f"  UID ({len(data)} bytes): {bytes(data).hex().upper()}")
    
    # 8. Read File Counters
    print_section("8. FILE COUNTERS")
    
    for file_no in [0x02, 0x03]:
        # GetFileCounters: 90 F6 00 00 01 [FileNo] 00
        apdu = [0x90, 0xF6, 0x00, 0x00, 0x01, file_no, 0x00]
    ...
    # 9. Authentication State Check
    print_section("9. AUTHENTICATION STATE")
    ...
    if success and data:
        ...
        # Check if we can access anything after Phase 1 (before Phase 2)
        print("\n  Testing commands after Phase 1 (before Phase 2):")
        
        # Try GetFileSettings after Phase 1
        print("    GetFileSettings(0x02) after Phase 1:")
        apdu_fs = [0x90, 0xF5, 0x00, 0x00, 0x01, 0x02, 0x00]
        fs_success, fs_data, fs_sw1, fs_sw2, fs_status = try_command(card, "GetFileSettings", apdu_fs)
        print(f"      {fs_status}")
        if fs_success and fs_data:
            print(f"      Data: {bytes(fs_data).hex().upper()}")
        
        # Try GetCardUID after Phase 1
        print("    GetCardUID (0x51) after Phase 1:")
        apdu_uid = [0x90, 0x51, 0x00, 0x00, 0x00]
        uid_success, uid_data, uid_sw1, uid_sw2, uid_status = try_command(card, "GetCardUID", apdu_uid)
        print(f"      {uid_status}")
        if uid_success and uid_data:
            print(f"      UID: {bytes(uid_data).hex().upper()}")
    
    # 10. Configuration Query
    print_section("10. CONFIGURATION QUERIES")
    ...
    print_section("11. CONNECTION INFORMATION")
    print(f"Reader: {card}")
