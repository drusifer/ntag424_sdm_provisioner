...
def provision_with_clean_types():
    """Provision using type-safe constants - no magic numbers!"""
    ...
    with CardManager(0) as card:
        ...
        # Option 1: Use preset access rights
        config = SDMConfiguration(
            ...
            enable_sdm=True,
            sdm_options=SDMOption.ENABLED | SDMOption.UID_MIRROR,
            picc_data_offset=34,
            mac_input_offset=34,
            mac_offset=47
        )
        
        # Option 2: Build custom access rights
        custom_rights = AccessRights(
            read=AccessRight.FREE,
            write=AccessRight.KEY_2,    # Different key!
            read_write=AccessRight.KEY_1,
        ...
        config = SDMConfiguration(
            ...
            access_rights=custom_rights,
            enable_sdm=True,
            sdm_options=SDMOption.BASIC_SDM,  # Preset combo
            picc_data_offset=34,
            mac_input_offset=34,
            mac_offset=47
        )
        
        print(config)  # Beautiful output with named enums!
        
        # Execute command
        ChangeFileSettings(config).execute(card, session=session)
        
        print("✅ Provisioned with type-safe configuration!")
