...
def main():
    ...
    try:
        with CardManager() as card:
            ...
            print_section("3. Phase 2 Comparison: Escape vs Transmit (No Delay)")
            
            # Test Phase 2 with escape mode (no delay)
            print("\n  === ESCAPE MODE (No Delay) ===")
            result_escape, sw1_esc2, sw2_esc2 = test_phase2_with_mode(
                card, encrypted_rndb, FACTORY_KEY, 
                use_escape=True, delay_ms=0, description=" (ESCAPE, no delay)"
            )
            
            # Small delay between tests
            time.sleep(1.0)
            
            # Test Phase 2 with transmit mode (no delay)
            print("\n  === TRANSMIT MODE (No Delay) ===")
            result_transmit, sw1_tx2, sw2_tx2 = test_phase2_with_mode(
                card, encrypted_rndb, FACTORY_KEY,
                use_escape=False, delay_ms=0, description=" (TRANSMIT, no delay)"
            )
            
            print_section("4. Phase 2 with Timing Delays")
