...
from ntag424_sdm_provisioner.constants import Ntag424VersionInfo, SDMConfiguration, CommMode
...
from ntag424_sdm_provisioner.constants import (
    ...
    AccessRightsPresets, SDMOption, FACTORY_KEY, SW_AUTH_FAILED,
    StatusWord, ErrorCategory, describe_status_word
)
...
def provision_with_error_handling():
    ...
    try:
        with CardManager(0) as card:
            ...
            version_info: Ntag424VersionInfo = GetChipVersion().execute(card)
            ...
            for key_no in [KeyNo.KEY_0, KeyNo.KEY_1, KeyNo.KEY_2, KeyNo.KEY_3, KeyNo.KEY_4]:
                try:
                    ...
                    factory_key_results[key_no] = "FAILED"
                    print(f"  Key {key_no}: Factory key failed - {e.sw1:02X}{e.sw2:02X} ({e.category.name})\n Error: {e}")
            ...
            try:
                ...
                print(f"  NDEF File: Error - {e.sw1:02X}{e.sw2:02X} ({e.category.name})")
            ...
            for file_no in [FileNo.CC_FILE, FileNo.PROPRIETARY_FILE]:
                try:
                    ...
                    print(f"  File {file_no}: Error - {e.sw1:02X}{e.sw2:02X} ({e.category.name})")
            ...
            try:
                ...
                print(f"  SDM Status: Requires authentication - {e.sw1:02X}{e.sw2:02X} ({e.category.name})")
            ...
            print(f"  CC File: File {FileNo.CC_FILE}")
            print(f"  NDEF File: File {FileNo.NDEF_FILE}")
            print(f"  Proprietary File: File {FileNo.PROPRIETARY_FILE}")
            ...
            print(f"SUCCESS: Re-authenticated with {KeyNo.KEY_0}")
            ...
            print(f"SUCCESS: SDM configuration applied to {FileNo.NDEF_FILE}")
            ...
            print(f"SUCCESS: NDEF message written to {FileNo.NDEF_FILE}")
        ...
        print(f"   Status Word: {e.status_word.name} (0x{e.sw1:02X}{e.sw2:02X})")
        print(f"   Category: {e.category.name}")
        
        if e.is_authentication_error():
            print("   -> Check your key! Authentication failed")
