...
def provision_with_error_handling():
    ...
    try:
        with CardManager(0) as card:
            ...
            session.authenticate(card, key_no=KeyNo.KEY_0, use_escape=True)
            ...
            ChangeKey(KeyNo.KEY_0, new_picc_key, FACTORY_KEY, use_escape=True).execute(card)
            print("✓ PICC Master Key changed")
            
            # Change File Data Read Key (Key 1)
            ChangeKey(KeyNo.KEY_1, new_file_key, FACTORY_KEY, use_escape=True).execute(card)
            print("✓ File Data Read Key changed")
            
            # Change SDM MAC Key (Key 2)
            ChangeKey(KeyNo.KEY_2, new_sdm_key, FACTORY_KEY, use_escape=True).execute(card)
            ...
            new_session.authenticate(card, key_no=KeyNo.KEY_0, use_escape=True)
            ...
            ChangeFileSettings(config, use_escape=True).execute(card, session=new_session)
            ...
            WriteData(FileNo.NDEF_FILE, 0, ndef_message, use_escape=True).execute(card)
