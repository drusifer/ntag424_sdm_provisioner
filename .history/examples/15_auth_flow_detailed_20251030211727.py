...
def main():
    ...
    try:
        with CardManager(0, timeout_seconds=15) as card:
            ...
            print("\n⚠️  NOTE: After Phase 2 failures, the tag may enter a locked state.")
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            print(f"\n✅✅✅ SUCCESS with {key_name}! ✅✅✅")
                            results.append((key_name, test_key, True, None))
                            break  # Found working key!
                            
                        except ApduError as e:
                            print(f"\n❌ Phase 2 failed with {key_name}")
                            print(f"   SW: {e.sw1:02X}{e.sw2:02X}")
                            results.append((key_name, test_key, False, (e.sw1, e.sw2)))
                            
                    except ApduError as e:
                        print(f"\n⚠️  Phase 1 failed (state issue): {e.sw1:02X}{e.sw2:02X}")
                        print(f"   This usually means previous Phase 2 failure affected tag state")
                        print(f"   Try removing and re-tapping tag, or skip to next key")
                        results.append((key_name, test_key, False, (e.sw1, e.sw2)))
                        # Don't break - continue with other keys
