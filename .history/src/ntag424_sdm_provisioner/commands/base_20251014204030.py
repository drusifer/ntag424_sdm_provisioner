...
class ApduCommand(ABC):
    ...
    def send_apdu(self, connection: NTag424CardConnection, apdu: List[int]) -> Tuple[List[int], int, int]:
        ...
        if self.use_escape:
            resp = connection.control(IOCTL_CCID_ESCAPE, apdu)

            print(f"Raw response {len(resp)} bytes: {hexb(resp)}" )

            # If response includes status bytes at end, print them separated:
            if isinstance(resp, (bytes, bytearray)) and len(resp) >= 2:
                sw1, sw2 = resp[-2:]
                data = resp[:-2]
                print(f"Possible SW1:{hexb(sw1)} SW2:{hexb(sw2)} Data:{hexb(data)}")
                return list(data), sw1, sw2
            else: 
                raise ApduError(f"APDU response[{hexb(resp)}] too short or invalid", 0x00, 0x00)
        else:
