...
class AuthenticateEV2First(ApduCommand):
    ...
    def execute(self, connection: 'NTag424CardConnection') -> AuthenticationChallengeResponse:
        ...
        # According to spec, this is complete - we have the challenge
        # But on Seritag, we may need to send GetAdditionalFrame to complete the transaction
        full_response = bytearray(data)
        
        # Try reading additional frames (may be needed for Seritag)
        while (sw1, sw2) == SW_ADDITIONAL_FRAME:
            log.debug("AuthenticateEV2First: Additional frame requested, sending GET ADDITIONAL FRAME...")
            af_apdu = [0x90, 0xAF, 0x00, 0x00, 0x00]
            data, sw1, sw2 = self.send_apdu(connection, af_apdu)
            full_response.extend(data)
            log.debug(f"Additional frame: data={len(data)} bytes, SW={sw1:02X}{sw2:02X}")
        
        # Final response should be SW=9000 or we should have the challenge
        if (sw1, sw2) not in [SW_OK, SW_OK_ALTERNATIVE]:
            log.warning(f"Phase 1 final frame unexpected SW={sw1:02X}{sw2:02X}, but continuing with challenge data")
        
        # Use first 16 bytes as the encrypted RndB (standard size)
