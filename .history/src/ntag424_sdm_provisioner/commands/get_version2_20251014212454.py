...
class GetVersion(ApduCommand):
    ...
    def execute(self, ncc: NTag424CardConnection) -> VersionInfo:
        """
        ...
        _, sw1, sw2 = self.send_apdu(ncc, self.SELECT_APDU)
        if sw1 != 0x90 or sw2 != 0x00:
            raise ApduError("Failed to select NTAG application", sw1, sw2)
        print("Success.")

        print("\nStep 2: Sending GetVersion command...")
        data, sw1, sw2 = self.send_apdu(ncc, self.GET_VERSION_APDU)
        if sw1 != 0x90 or sw2 != 0x00:
