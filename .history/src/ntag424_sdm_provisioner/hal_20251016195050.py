...
class CardManager:
    """
    A simplified, self-contained context manager that uses a direct,
    blocking call to wait for a card tap. This avoids all threading complexity.
    """
    ...
    def __enter__(self) -> 'NTag424CardConnection':
        try:
            ...
            # STEP 1: Set up the state we are waiting for.
            # We start in an 'unaware' state and wait for a card to become 'present'.
            reader_states = [(str(reader), SCARD_STATE_UNAWARE)]

            # STEP 2: Make the blocking call.
            # This function will pause here until a card is tapped or the timeout is hit.
            current_states = SCardGetStatusChange(reader_states, self.timeout_ms)
            (reader_name, event_state, atr) = current_states[0]

            # STEP 3: Check the result.
            if not (event_state & SCARD_STATE_PRESENT):
                # This means the timeout was reached without a card tap.
                raise NTag242ConnectionError("Timeout: No card was presented.")
            
            log.info(f"Card detected with ATR: {hexb(atr)}. Connecting...")

            # STEP 4: Now that we know a card is present, connect to it.
