...
class Ntag424Session:
    ...
    * To create the **Encryption key**, we do the same, but with the different 
    ...
    def authenticate(self) -> None:
        ...
        self._rndB = self.connection.execute(auth1)
        print("INFO: Authentication Part 1 successful.")

        # Part 2: PICC authenticates to Backend
        # Pass the raw key bytes; AuthenticateEV2Part2 now manages its own
        # fresh cipher instances to avoid stateful encrypt/decrypt errors.
        auth2 = AuthenticateEV2Part2(self._key, self._rndB)
        self.connection.execute(auth2)
