...
from smartcard.Exceptions import NoReadersError, CardConnectionException
...
def list_readers() -> List[str]:
    ...
    if not reader_list:
        ...
        raise NoReadersError("No readers available")
...
def get_atr(connection: CardConnection) -> str:
    """
...
def _main():
    ...
    try:
        ...
        with CardManager(reader_index=0) as conn:
            ...
            atr_hex = get_atr(conn)
            print(f"  Card ATR: {atr_hex}")

    except (NoReadersError, CardConnectionException, IndexError) as e:
        print(f"\nError: {e}")
