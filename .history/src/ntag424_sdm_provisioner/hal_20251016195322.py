...
# file: hal.py (or wherever your CardManager is)
...
# pyscard imports
from smartcard.System import readers
from smartcard.Exceptions import NoReadersException, CardConnectionException, CardRequestTimeoutException
from smartcard.CardConnection import CardConnection

# ✅ Import the blocking function and necessary constants
...
class CardManager:
    ...
    def __init__(self, reader_index: int = 0, timeout_seconds: int = 15):
        # We don't need reader_index anymore, but keep it for API consistency
        self.timeout = timeout_seconds
        self.connection: Optional[CardConnection] = None

    def __enter__(self) -> 'NTag424CardConnection':
        try:
            # Log available readers for user convenience
            log.info(f"Available readers: {[str(r) for r in readers()]}")
            log.info(f"Waiting up to {self.timeout} seconds for a card tap...")

            # STEP 1: Create a CardRequest with a timeout.
            card_request = CardRequest(timeout=self.timeout)

            # STEP 2: Block until a card is presented.
            # This single call handles everything: waiting, detecting, and connecting.
            cardservice = card_request.waitforcard()
            
            # The connection is now stored in the cardservice object
            ...
            # This is the specific exception thrown on timeout.
