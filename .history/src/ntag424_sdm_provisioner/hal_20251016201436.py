...
class CardManager:
    ...
    def __enter__(self) -> 'NTag424CardConnection':
        try:
            ...
            raise NTag242ConnectionError("Timeout: No card was presented.",e)
        except IndexError as e:
            log.error(f"Reader index {self.reader_index} is out of bounds.")
            raise NTag242ConnectionError(f"Invalid reader index.", e)
        except Exception as e:
            log.error(f"Failed to establish card connection: {e}", e)
            raise NTag242ConnectionError(f"Failed to connect: {e}", e)
