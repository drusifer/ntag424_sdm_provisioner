...
class Ntag424AuthSession:
    ...
    def _parse_card_response(self, encrypted_response: bytes, rnda: bytes) -> AuthenticationResponse:
        ...
        # Format: Ti (4 bytes) || RndA' (16 bytes) || PDcap (variable) || PCDcap (variable)
        if len(decrypted_response) < 20:  # Minimum: Ti(4) + RndA'(16)
        ...
        # For NTAG424, PDcap and PCDcap are typically minimal or empty
        # The exact format may vary, so we'll handle what's available
        remaining_data = decrypted_response[20:]
        
        # Simple parsing: assume minimal PDcap/PCDcap for now
        # In a full implementation, you'd parse these according to the spec
        pdcap = b''  # Placeholder - would need proper parsing
        pcdcap = b''  # Placeholder - would need proper parsing
