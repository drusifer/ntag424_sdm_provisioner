...
# Compatibility helper: module-level send_apdu used by command implementations.
def send_apdu(connection: CardConnection, apdu: List[int]) -> Tuple[List[int], int, int]:
    """
    Send a raw APDU using either a wrapped NTag424CardConnection or a raw
    pyscard/CardConnection-like object (including mocks).

    This restores the historical API where commands call `hal.send_apdu(conn, apdu)`.
    """
    # If the connection has a high-level send_apdu (our NTag424CardConnection), use it.
    if hasattr(connection, "send_apdu"):
        return connection.send_apdu(apdu)

    # Otherwise assume it's a raw pyscard CardConnection or a mock implementing transmit()
...
class NTag424CardConnection():
    ...
    def execute(self, command):
        """Execute an APDU command object against the underlying card connection.

        This method centralizes command execution so callers can write
        `connection.execute(command)` instead of `command.execute(connection)`.
        The command implementation is expected to accept a raw pyscard
        CardConnection when executed.
        """
        return command.execute(self.connection)
