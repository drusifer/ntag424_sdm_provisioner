...
def attempt_factory_key_variations():
    ...
    possible_keys = [
        ...
        b'\xFF' * 16,  # All FF
        b'SERITAG424DNA\x00\x00\x00',  # Seritag branded key (16 bytes)
        b'seritag424dna\x00\x00\x00\x00\x00',  # Lowercase (16 bytes)
        b'Seritag424DNA\x00\x00\x00\x00',  # Mixed case (16 bytes)
        bytes.fromhex('0123456789ABCDEFFEDCBA0987654321'),  # Sequential pattern (32 chars = 16 bytes)
        bytes.fromhex('DEADBEEFCAFEBABE1234567890ABCDEF'),  # Common test pattern
        b'SeritagNTAG424\x00',  # Alternative branding (16 bytes)
    ]
