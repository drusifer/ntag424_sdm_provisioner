...
- **Analysis**: Command format incorrect or LRP not enabled
- **Note**: May need different command format for LRP

### **AuthenticateEV2NonFirst (0x77)**
- **Status**: ❌ Failed
- **SW (direct)**: 919D (Permission Denied / Not Enabled)
- **SW (after Phase 1)**: 91CA (Command Aborted)
- **Analysis**: 
  - Command exists but requires completed transaction
  - After Phase 1, tag expects Phase 2, not new 0x77
  - 0x77 likely for subsequent authentications in same transaction (after Phase 1+2 complete)

### **Command 0x51 Variations**
- **Variation 1** (32 bytes data): SW=91CA (Command Aborted)
- **Variation 2** (P1=0x01): SW=6A86 (Wrong P1/P2)
- **Variation 3** (no data): SW=91AE (Authentication Error)
- **Analysis**: Command recognized, but format/state requirements unclear
- **Note**: SW=91AE suggests auth-related function

---

**Status**: 🔍 Testing alternatives - SW=919D for 0x77 suggests it needs prior auth
